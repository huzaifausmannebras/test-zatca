const StandardAdvancePaymentForeign = async data => {
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2">
    ...
    <cac:TaxTotal>
        <cbc:TaxAmount currencyID="SAR">${body?.taxdocumentamount?.toFixed(2)}</cbc:TaxAmount>
    </cac:TaxTotal>
    <cac:TaxTotal>
    ...
    <cac:LegalMonetaryTotal>
        <cbc:LineExtensionAmount currencyID="${body?.currencyno}">${body?.documentamountexclusivetax?.toFixed(
            2,
        )}</cbc:LineExtensionAmount>
        <cbc:TaxExclusiveAmount currencyID="${body?.currencyno}">${body?.documentamountexclusivetax?.toFixed(
            2,
        )}</cbc:TaxExclusiveAmount>
        <cbc:TaxInclusiveAmount currencyID="${body?.currencyno}">${body?.paymentdocamount?.toFixed(2)}</cbc:TaxInclusiveAmount>
        <cbc:AllowanceTotalAmount currencyID="${body?.currencyno}">0.00</cbc:AllowanceTotalAmount>
        <cbc:PayableAmount currencyID="${body?.currencyno}">${body?.paymentdocamount?.toFixed(2)}</cbc:PayableAmount>
        </cac:LegalMonetaryTotal>
       <cac:InvoiceLine>
        <cbc:ID>1</cbc:ID>
...
module.exports = StandardAdvancePaymentForeign;

//  <cbc:PrepaidAmount currencyID="SAR">${body?.grossdocamount?.toFixed(2)}</cbc:PrepaidAmount>
//   <cbc:IssueDate>
